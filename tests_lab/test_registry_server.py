...
async def test_accuracy_milestone_exposes_compact_readonly_baseline(tmp_path):
    ...
    evidence_path = tmp_path / "docs/evidence/daily-mean-reversion-exit-search.json"
    evidence_path.parent.mkdir(parents=True)
    evidence = {
        "status": "stopped_no_qualifying_geometry",
        "eligible_for_live": False,
        "runs": [{"rule": "rsi2<5 & above ema200", "test_profitable_rate": 0.5013}],
    ...
    context_path = tmp_path / "docs/evidence/aem-market-sector-context-readiness.json"
    context = {
